...
class EvolveGCN(torch.nn.Module):
    def __init__(
        ...
        **_kwargs
        ...
        assert (
            input_dim == hidden_dim and hidden_dim == output_dim
        ), "EvolveGCN requires same input and output dimension"
        dim = input_dim
        ...
        for _ in range(num_layers):
            if rnn == "LSTM":
                layer = torch_geometric_temporal.nn.EvolveGCNO(
                    dim, normalize=False, add_self_loops=False
                )
            elif rnn == "GRU":
                layer = torch_geometric_temporal.nn.EvolveGCNH(
                    num_nodes, dim, normalize=False, add_self_loops=False
                )
            else:
                raise NotImplementedError("no such RNN model {}".format(rnn))

            self.layers.append(layer.to(device))
            self.norms.append(torch.nn.BatchNorm1d(dim, device=device))
