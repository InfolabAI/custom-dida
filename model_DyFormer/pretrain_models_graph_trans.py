...
def train_current_time_step(
    FLAGS, graphs, adjs, device, res_id=None, test_pedges=None, test_nedges=None
):
    """
    Setup
    """
    ...
    # feats (list) 의 각 원소는 sparse matrix 이고 [#nodes, #nodes] 의 shape 을 가짐
    ...
    ###########################################################
    data_dict = compute_node_edge_encoding(
        graphs_train, FLAGS, force_regen=FLAGS.force_regen
    )
    eval_edge_encode, eval_PPR, eval_edge_dist_encode = data_dict[
        "st_%d_et_%d" % (train_start_time, eval_time)
    ]
    ...
    test_edges = test_pedges.t().numpy()
    test_edges_false = test_nedges.t().numpy()

    all_edge_num = len(train_edges) + len(val_edges) + len(test_edges)
    if all_edge_num > 1000:
        sample_ratio = 1000 / all_edge_num
        train_edges = train_edges[
            np.random.permutation(len(train_edges))[
                : int(sample_ratio * len(train_edges))
            ]
        ]
        train_edges_false = train_edges_false[
            np.random.permutation(len(train_edges_false))[
                : int(sample_ratio * len(train_edges_false))
            ]
        ]
        val_edges = val_edges[
            np.random.permutation(len(val_edges))[: int(sample_ratio * len(val_edges))]
        ]
        val_edges_false = val_edges_false[
            np.random.permutation(len(val_edges_false))[
                : int(sample_ratio * len(val_edges_false))
            ]
        ]
        test_edges = test_edges[
            np.random.permutation(len(test_edges))[
                : int(sample_ratio * len(test_edges))
            ]
        ]
        test_edges_false = test_edges_false[
            np.random.permutation(len(test_edges_false))[
                : int(sample_ratio * len(test_edges_false))
            ]
        ]
    ...
    ############ Used for DyGraphTransformer ##################
    ###########################################################
    activate_eval_nodes = np.concatenate(
        [
            train_edges,
            train_edges_false,
            val_edges,
    ...
    activate_eval_nodes_splits = np.array_split(
        activate_eval_nodes, math.ceil(num_activate_eval_nodes / FLAGS.eval_batch_size)
    )
    print("Eval split size", len(activate_eval_nodes_splits))

    ### get new indicies
    eval_all_nodes_to_new_index = dict(
        zip(activate_eval_nodes, np.arange(num_activate_eval_nodes))
    )
    train_edges_new = translate(
        train_edges, eval_all_nodes_to_new_index, shape="Nx2"
    )  # edges [Nx2] -> [2xN]
    train_edges_false_new = translate(
        train_edges_false, eval_all_nodes_to_new_index, shape="Nx2"
    )
    val_edges_new = translate(val_edges, eval_all_nodes_to_new_index, shape="Nx2")
    val_edges_false_new = translate(
        val_edges_false, eval_all_nodes_to_new_index, shape="Nx2"
    )
    test_edges_new = translate(test_edges, eval_all_nodes_to_new_index, shape="Nx2")
    test_edges_false_new = translate(
        test_edges_false, eval_all_nodes_to_new_index, shape="Nx2"
    )
    ...
    best_valid_model_path = os.path.join(
        output_dir, "best_valid_model_{}.pt".format(FLAGS.dataset)
    )
    ...
    for epoch in range(FLAGS.num_epoches):
        ...
        while not minibatchIterator.end():
            ...
            (
                ...
                train_end,
                batch_nodes,
            ) = minibatchIterator.next_minibatch_feed_dict()

            # get spatial-temporal encoding
            edge_encode, PPR, edge_dist_encode = data_dict[
                "st_%d_et_%d" % (train_start, train_end)
            ]
            ...
            (
                node_feats_np_1,
                edge_encode_np_1,
                edge_dist_encode_np_1,
                target_node_size_1,
                context_nodes_size_1,
                _,
            ...
            # forward propagation
            output_1, output_unsupervised_1 = model(
                node_feats_th,
                edge_encode_th,
                edge_dist_encode_th,
                target_node_size_1,
            ...
            (
                node_feats_np_2,
                edge_encode_np_2,
                edge_dist_encode_np_2,
                target_node_size_2,
                context_nodes_size_2,
                _,
            ) = get_common_neighbors(
                active_nodes,
                edge_encode,
                ...
                feats[train_end - 1 - train_start],
                FLAGS,
                deterministic_neighbor_sampling=False,
            )
            ...
            # forward propagation
            output_2, output_unsupervised_2 = model(
                node_feats_th,
                edge_encode_th,
                edge_dist_encode_th,
                target_node_size_2,
            ...
            # First view
            sampled_temporal_edges = generate_temporal_edges(
                edge_encode_np_1, target_node_size_1, FLAGS.neg_sample_size
            )

            for t, (pos_edges, neg_edges) in enumerate(sampled_temporal_edges):
                loss += link_forecast_loss(
                    output_unsupervised_1[t, :, :],
                    pos_edges,
                    neg_edges,
                    FLAGS.neg_weight,
                    device,
                ) / output_unsupervised_1.size(0)

            # Second view
            sampled_temporal_edges = generate_temporal_edges(
                edge_encode_np_2, target_node_size_2, FLAGS.neg_sample_size
            )

            for t, (pos_edges, neg_edges) in enumerate(sampled_temporal_edges):
                loss += link_forecast_loss(
                    output_unsupervised_2[t, :, :],
                    pos_edges,
                    neg_edges,
                    FLAGS.neg_weight,
                    device,
                ) / output_unsupervised_2.size(0)

            loss += multi_view_loss(
                output_1[:target_node_size_1, :],
                output_2[:target_node_size_2, :],
                model.BYOL_decoder,
            )
        ...
        logger.info("Pre Train Loss at epoch {} : {}".format(epoch, epoch_train_loss))
        logger.info(f"Pre Time for epoch {epoch_time}")
        ...
        if (epoch + 1) % FLAGS.test_freq == 0:
            ...
            for cur_activate_eval_nodes in activate_eval_nodes_splits:
                (
                    ...
                    target_node_size_1,
                    context_nodes_size_1,
                    eval_all_nodes_to_new_index,
                ) = get_common_neighbors(
                    cur_activate_eval_nodes,
                    eval_edge_encode,
                    eval_PPR,
            ...
            val_results, test_results, _, _ = evaluate_classifier(
                train_edges_new,
                train_edges_false_new,
                val_edges_new,
                val_edges_false_new,
                test_edges_new,
                test_edges_false_new,
                emb,
                emb,
            )
            ...
            print(
                "Epoch %d, Val AUC_HAD %.4f, Test AUC_HAD %.4f, Val AUC_SIGMOID %.4f, Test AUC_SIGMOID %.4f, "
                % (epoch, val_HAD, test_HAD, val_SIGMOID, test_SIGMOID)
            )
    ...
    val_results, test_results = (
        epochs_val_result["HAD"][best_epoch],
        epochs_test_result["HAD"][best_epoch],
    )
    ...
    with open(
        os.path.join(output_dir, "result_{}.json".format(FLAGS.dataset)), "w"
    ) as outfile:
    ...
    with open(
        os.path.join(output_dir, "result_emb_edge_{}.pkl".format(FLAGS.dataset)), "wb"
    ) as outfile:
