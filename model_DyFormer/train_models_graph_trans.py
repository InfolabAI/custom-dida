...
def train_current_time_step(
    ...
    print(
        train_edges.shape,
        train_edges_false.shape,
        val_edges.shape,
        val_edges_false.shape,
        test_edges.shape,
        test_edges_false.shape,
    )
    ...
    for epoch in range(FLAGS.num_epoches):
        ...
        while not minibatchIterator.end():
            if FLAGS.supervised:
                ...
                ###
                output, output_unsupervised = model(
                    node_feats_th,
                    edge_encode_th,
                    edge_dist_encode_th,
                    target_node_size,
                ...
                if FLAGS.supervised_loss:
                    pos_edges_new = translate(
                        pos_edges, all_nodes_to_new_index, shape="2xN"
                    )
                    neg_edges_new = translate(
                        neg_edges, all_nodes_to_new_index, shape="2xN"
                    )
                    loss += link_forecast_loss(
                        output, pos_edges_new, neg_edges_new, FLAGS.neg_weight, device
                    )

                if FLAGS.unsupervised_loss:
                    sampled_temporal_edges = generate_temporal_edges(
                        edge_encode_np, target_node_size, FLAGS.neg_sample_size
                    )
                    # print(output_unsupervised.shape)
                    for t, (pos_edges, neg_edges) in enumerate(sampled_temporal_edges):
                        loss += link_forecast_loss(
                            output_unsupervised[t, :, :],
                            pos_edges,
                            neg_edges,
                            FLAGS.neg_weight,
                            device,
                        ) / output_unsupervised.size(0)
        ...
        # logging
        logging.info(
            "Mini batch Iter: {} train_loss= {:.5f}".format(it, loss.data.item())
        )
        ...
        logger.info(
            "Finetune Train Loss at epoch {} : {}".format(
                epoch, np.mean(epoch_train_loss)
            )
        )
        logger.info(f"Finetune Time for epoch {epoch_time}")
    ...
    np.save(
        os.path.join(output_dir, "test_pred_true.npy"),
        np.array(best_valid_epoch_predict_true),
    )
...
def get_common_neighbors(
    ...
    feats,
    FLAGS,
    deterministic_neighbor_sampling=None,
):
    ...
    ###
    edge_dist_encode_np_1 = np.ones((all_node_size, all_node_size)) * (
        FLAGS.max_dist + 1
    )
    edge_dist_encode_tmp = edge_dist_encode[all_nodes, :][:, all_nodes].tocoo()
    edge_dist_encode_np_1[
        edge_dist_encode_tmp.row, edge_dist_encode_tmp.col
    ] = edge_dist_encode_tmp.data
