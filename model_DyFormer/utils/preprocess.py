...
def create_data_splits(adj, next_adj, val_mask_fraction=0.2, test_mask_fraction=0.6):
    ...
    edges_next = np.array(
        list(set(nx.from_scipy_sparse_array(next_adj).edges()))
    )  # all edges in testing snapshot
...
def get_evaluation_data(
    last_train_adj, eval_adj, eval_time_step, dataset, force_regen=False
):
    """
    ...
    if force_regen == False:
        try:
            ...
            (
                ...
                last_train_adj, eval_adj, val_mask_fraction=0.2, test_mask_fraction=0.6
            )
            np.savez(
                eval_path,
                data=np.array(
    ...
    return (
        ...
        np.array(train_edges_false),
        np.array(val_edges),
        np.array(val_edges_false),
        np.array(test_edges),
        np.array(test_edges_false),
    )
