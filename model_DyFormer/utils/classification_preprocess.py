...
def cls_load_graphs(dataset_str):
    ...
    """
    graphs = np.load(
        "./model_DyFormer/data/{}/{}".format(dataset_str, "graphs.npz"),
        allow_pickle=True,
    )["graph"]
...
def cls_load_feats(dataset_str):
    ...
    """
    features = np.load(
        "./model_DyFormer/data/{}/{}".format(dataset_str, "features.npz"),
        allow_pickle=True,
    )["feats"]
...
def cls_get_context_pairs(
    graphs_train, eval_time_step, window_size, dataset, force_regen=False
):
    """
    ...
    """
    load_path = "./model_DyFormer/data/{}/train_pairs_n2v_{}_{}.pkl".format(
        dataset, str(eval_time_step), str(window_size)
    )
...
def cls_create_data_splits(
    evaluation_graph, num_time_steps, val_mask_fraction=0.2, test_mask_fraction=0.2
):
    """
    ...
    new_ids = np.concatenate(
        (all_positives_ids, all_negatives_ids)
    )  # new reconstructed index list for nodes
    new_labels = np.concatenate(
        (np.zeros_like(all_positives_ids), np.ones_like(all_negatives_ids))
    )
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        new_ids, new_labels, stratify=new_labels, train_size=train_frac
    )
    # split val / test
    x_val, x_test, y_val, y_test = train_test_split(
        ...
        y_test,
        stratify=y_test,
        train_size=val_mask_fraction / (val_mask_fraction + test_mask_fraction),
    )
...
def cls_get_evaluation_data(
    evaluation_graph, num_time_steps, dataset, force_regen=False
):
    """
    ...
    """
    eval_path = "./model_DyFormer/data/{}/eval_{}.npz".format(
        dataset, str(num_time_steps)
    )

    if force_regen == False:
        try:
            x_train, y_train, x_val, y_val, x_test, y_test = np.load(
                eval_path, encoding="bytes", allow_pickle=True
            )["data"]
            print("Loaded eval data")
        except (OSError, IOError):
            print("Generating and saving eval data (cls)....")
            x_train, y_train, x_val, y_val, x_test, y_test = cls_create_data_splits(
                evaluation_graph,
                num_time_steps,
                val_mask_fraction=0.2,
                test_mask_fraction=0.2,
            )
            np.savez(
                eval_path,
                data=np.array([x_train, y_train, x_val, y_val, x_test, y_test]),
            )
    else:
        ...
        np.savez(
            eval_path, data=np.array([x_train, y_train, x_val, y_val, x_test, y_test])
        )
...
def cls_process_graph(G):
    ...
    for u, data in new_G.nodes(data=True):
        weights = np.array(
            [data["weight"] for (i, j, data) in new_G.edges(u, data=True)]
        )
...
def cls_get_feats(adjs_train, num_nodes_graph_eval, train_start_time, eval_time, FLAGS):
    if FLAGS.feature_less == True:
        ...
        for adj in adjs_train:
            ...
            cur_feats = csr_matrix(
                (np.ones_like(cur_node_ids), (cur_node_ids, cur_node_ids)),
                shape=(cur_num_nodes, num_nodes_graph_eval),
                dtype=np.float32,
            )
