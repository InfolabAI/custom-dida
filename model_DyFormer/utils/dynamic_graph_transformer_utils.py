...
def generate_compressed_graphs(graphs, alpha=0.15, max_dist=5):  # Checked
    ...
    for t, G in enumerate(graphs[::-1]):
        ...
        edge_encode_list.append(
            csr_matrix(
                (edge_encode_data, (edge_encode_row, edge_encode_col)),
    ...
    #### PPR
    st = time.time()
    sparse_edge_exist = csr_matrix(
        (edge_encode_all_data, (edge_encode_all_row, edge_encode_all_col)),
        shape=(num_nodes, num_nodes),
    )
...
def compute_shortest_path_dist(adj, dmax):
    ...
    short_path_dist = csr_matrix(
        (short_path_dist_data, (short_path_dist_row, short_path_dist_col)),
        shape=(num_nodes, num_nodes),
    )
...
def compute_node_edge_encoding(graphs_train, FLAGS, force_regen=False):
    compact_edges_dir = "./model_DyFormer/data/%s/compact_edges_et%d_wz%d.pkl" % (
        FLAGS.dataset,
        FLAGS.eval_time,
        FLAGS.cur_window,
    )
    ...
    if os.path.exists(compact_edges_dir):
        ...
        for i in range(1, num_graphs + 1):
            edge_encode, PPR, edge_dist_encode = generate_compressed_graphs(
                graphs_train[0:i], max_dist=FLAGS.max_dist
            )
            dict_name = "st_%d_et_%d" % (
                FLAGS.train_start_time,
                FLAGS.train_start_time + i,
            )
            print(dict_name)
            data_dict[dict_name] = [edge_encode, PPR, edge_dist_encode]
        # 마지막 파일명 제외하고 폴더 생성
        os.makedirs(os.path.join(*compact_edges_dir.split("/")[:-1]), exist_ok=True)
...
def translate(edges, node_trans_dict, shape):
    ...
    if shape == "Nx2":
        for i in range(edges.shape[0]):
            new_edges.append(
                [node_trans_dict[edges[i, 0]], node_trans_dict[edges[i, 1]]]
            )
        new_edges = np.array(new_edges)
    elif shape == "2xN":
        for i in range(edges.shape[1]):
            new_edges.append(
                [node_trans_dict[edges[0, i]], node_trans_dict[edges[1, i]]]
            )
...
def sample_joint_neighbors(
    active_nodes, PPR, deterministic=True, two_stream_structure=True, max_neighbors=-1
):
    ...
    if two_stream_structure:
        ...
        sampling_prbs[active_nodes] = 0.0
        ...
        if deterministic:
            ...
            shared_neighbors = np.random.choice(
                PPR.shape[0], p=sampling_prbs, size=num_neighbors, replace=False
            )

        num_shared_neighbors = len(shared_neighbors)
        if num_shared_neighbors < num_active_nodes:
            select = np.random.permutation(num_active_nodes)[
                : num_active_nodes - num_shared_neighbors
            ]
...
def generate_temporal_edges(edge_encode_np, target_node_size, neg_sample_size):
    ...
    for t in range(time_steps):
        ...
        select = np.random.permutation(num_neg_samples)[
            : neg_sample_size * num_pos_samples
        ]
