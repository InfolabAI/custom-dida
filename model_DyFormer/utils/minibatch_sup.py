...
class NodeMinibatchIterator:
    def __init__(
        ...
        self.edges, self.num_edges = get_all_positive_edges(
            self.adjs[1:], self.num_nodes[:-1]
        )
    ...
    def end(
        ...
        return (self.cur_train_step + 1 >= self.num_graphs - 1) and (
            self.batch_num * self.batch_size >= self.num_edges[-1]
        )
    ...
    def next_minibatch_feed_dict(
        ...
        pos_edges = self.edges[self.cur_train_step][
            :, start_idx:end_idx
        ]  # [2, batch_size]
        num_nodes = self.num_nodes[self.cur_train_step]

        pos_edges, neg_edges = get_non_existing_edges(
            pos_edges, num_nodes, self.negative_mult_training
        )

        self.batch_num += 1
        if (self.batch_num) * self.batch_size >= self.num_edges[
            self.cur_train_step
        ] and (self.cur_train_step + 1 < self.num_graphs - 1):
    ...
    def shuffle(
        ...
        self.batch_num = 0
        self.edges = [
            edges[:, np.random.permutation(num_edges)]
            for num_edges, edges in zip(self.num_edges, self.edges)
        ]
