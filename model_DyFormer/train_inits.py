...
from .utils.dytrans_compact_utils import (
    create_compact_graph,
    get_compact_adj_edges,
    align_output,
)
...
# for boosting model
from .utils.temporal_boosting_utils import (
    get_normalized_eigenvalues,
    prepare_train_spectrums,
)
...
def create_save_path(FLAGS):
    ...
    with open(
        os.path.join(output_dir, "flags_{}.json".format(FLAGS.dataset)), "w"
    ) as outfile:
...
def generate_context_pairs(graphs, adjs, FLAGS):
    for time_step in range(FLAGS.min_time, FLAGS.max_time + 1):
        ...
        context_pairs_train = get_context_pairs(
            graphs_train,
            eval_time,
            FLAGS.cur_window + 1,
            FLAGS.dataset,
        ...
        print(
            "{} at timestep {} finished processing train & eval data with window size".format(
                FLAGS.dataset, str(time_step), str(FLAGS.cur_window)
            )
        )


def get_train_time_interval(
    FLAGS,
):
    ...
    # 보통 time step 을 1부터 시작하는 것으로 입력할 것이라 예상하고 -1
    eval_start_time = FLAGS.time_step - 1

    print(
        "Predict the %d-th graph (start from 0) using %d historical graphs"
        % (eval_start_time, window)
    )
