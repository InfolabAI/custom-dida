...
class RelTemporalEncoding(nn.Module):
    ...
    def __init__(self, n_hid, max_len=500, dropout=0.2):  # original max_len=240
...
class DGNNLayer(nn.Module):
    ...
    def __init__(
        ...
        self.cs_mlp = nn.Sequential(
            nn.Linear(hid_dim, 2 * hid_dim), nn.GELU(), nn.Linear(2 * hid_dim, hid_dim)
        )
    ...
    def collect_neighbors(self, edge_index_list, x_list):
        ...
        for t_tar in range(twin):
            ...
            for t_src in range(t_tar + 1):
                ...
                x_tar_e = x_list[t_tar].index_select(
                    self.node_dim, ei_src[1, :]
                )  # features for edge target node in time t_tar
                x_src_e = x_list[t_src].index_select(
                    self.node_dim, ei_src[0, :]
                )  # features for edge source node in time t_src
    ...
    def DAttn(self, x_tar, x_src, t_tar, t_src):
        ...
        if self.use_RTE:
            ...
            target_node_vec = self.time_emb(
                target_node_vec, torch.LongTensor([t_tar]).to(device)
            )
            source_node_vec = self.time_emb(
                source_node_vec, torch.LongTensor([t_src]).to(device)
            )
    ...
    def forward(self, edge_index_list, x_list):
        ...
        cs = []
        ss = []
        for t_tar, (x_tar, topos) in enumerate(
            self.collect_neighbors(edge_index_list, x_list)
        ):
    ...
    def __init__(self, args, data_to_prepare):
        ...
        self.layers = nn.ModuleList(
            DGNNLayer(
                ...
                sample_r=0.1,
                use_fmask=args.fmask,
                only_causal=args.only_causal,
            )
        ...
        self.ss_decoder = (
            LinkPredictor(2 * hid_dim, hid_dim, 1, 1, 0)
            if args.learns
            else MultiplyPredictor()
        )
