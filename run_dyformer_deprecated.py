from model_DyFormer.train_inits import *
from model_DyFormer.pretrain_models_graph_trans import (
    train_current_time_step as pretrain_train_current_time_step,
)
from model_DyFormer.train_models_graph_trans import (
...
def get_pretrain_model_path(FLAGS, res_id, seed=None):
    ...
    output_dir = "./model_DyFormer/all_logs/%s" % (FLAGS.model_name)
    return os.path.join(
        output_dir, FLAGS.res_id, "best_valid_model_{}.pt".format(FLAGS.dataset)
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(best_valid_model_path) == False:
        ...
        best_valid_model_path = pretrain_train_current_time_step(
            FLAGS, graphs, adjs, device, RES_ID
        )
    ...
    finetune_train_current_time_step(
        FLAGS, graphs, adjs, device, RES_ID, model_path=best_valid_model_path
    )
    torch.cuda.empty_cache()
