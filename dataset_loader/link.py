...
class LinkDatasetTemplate(DatasetTemplate):
    ...
    def process(self):
        ...
        for graph in self.graphs:
            graph.ndata["X"] = torch.randn(
                self.num_nodes, self.input_dim, device=self.device
            )
...
class BitcoinAlpha(LinkDatasetTemplate):
    def __init__(self, *args, **kwargs):
        super().__init__(
            "BitcoinAlpha",
            "https://cs.stanford.edu/~srijan/rev2/rev2data.zip",
            *args,
            **kwargs
        )
...
class WikiElec(LinkDatasetTemplate):
    def __init__(self, *args, **kwargs):
        super().__init__(
            "WikiElec",
            "https://snap.stanford.edu/data/wikiElec.ElecBs3.txt.gz",
            *args,
            **kwargs
        )
    ...
    def preprocess(self):
        with gzip.open(
            self.raw_path + "/wikiElec.ElecBs3.txt.gz", "rt", encoding="latin-1"
        ) as file:
            upper_bound = 2e9
            ...
            for line in file.readlines():
                ...
                if line[0] == "V":
                    ...
                    timestamp = time.mktime(
                        dateutil.parser.parse(timestamp).timetuple()
                    )
                ...
                raw_edges.append(
                    {
                        ...
                        "time": time.mktime(
                            dateutil.parser.parse(row["TIMESTAMP"]).timetuple()
                        ),
                    }
                )
